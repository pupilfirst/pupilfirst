...
describe Founders::UpdatePendingPaymentService do
  ...
  describe '#update' do
    context 'when the pending payment is in requested state' do
      ...
      let(:payment) { create :payment, :requested, amount: Startups::FeeAndCouponDataService.new(startup).emi }

      context 'when the amount is unchanged' do
        it 'verifies and returns payment' do
          allow(Instamojo::VerifyPaymentRequestService).to receive(:new).with(payment).and_return(verify_payment_request_service)
      ...
      context 'when a coupon has been applied and amount changed as a result' do
        before do
        ...
        it 'rebuilds payment' do
          ...
          allow(Instamojo::RequestPaymentService).to receive(:new).with(disabled_payment).and_return(request_payment_service)
    ...
    context 'when the pending payment is fresh' do
      ...
      it 'creates new Instamojo payment request' do
        allow(Instamojo::RequestPaymentService).to receive(:new).with(payment).and_return(request_payment_service)
        allow(request_payment_service).to receive(:request).and_return(updated_payment)
