...
describe "Postmark webhook bounce reports" do
  ...
  let(:headers) do
    request_spec_headers("hook_id_for_test", "hook_secret_for_test")
  end
    ...
    it "creates a bounce report of type hard-bounced" do
      expect do
        post "/users/email_bounce",
             ...
             headers: headers
      end.to change { BounceReport.where(email: user_1.email).count }.from(
        0
      ).to(1)
  ...
  context "when postmark reports some complaint from a user who is not registered" do
    it "saves the bounce report" do
      ...
      expect(BounceReport.where(email: "missinguser@example.com").count).to eq(
        1
      )
