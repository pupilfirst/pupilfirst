...
let make = (~items, ~multiSelectCB, _children) => {
  ...
  reducer: (action, _state) =>
    switch (action) {
    | UpdateSearchKey(searchKey) => ReasonReact.Update({searchKey: searchKey})
    },
  render: ({state, send}) => {
    ...
    let filteredList =
      ...
      |> List.filter(((_key, value, _)) =>
           Js.String.includes(String.lowercase(state.searchKey), String.lowercase(value))
         );
    <div>
      ...
      {nonSelectedList |> List.length > 0 ?
         <div className="flex relative">
           <div className="select-list__group bg-white border rounded rounded-t-none shadow pb-2 w-full">
             {nonSelectedList |> List.length > 3 ?
                <div className="px-3 pt-3 pb-2">
                  <input
                    ...
                    onChange={event => send(UpdateSearchKey(ReactEvent.Form.target(event)##value))}
                  />
                </div> :
                ReasonReact.null}
             <div className={nonSelectedList |> List.length > 3 ? "h-24 overflow-y-scroll" : ""}>
               {filteredList
                |> List.map(((_key, value, _)) =>
                     <div
                       key={_key |> string_of_int}
