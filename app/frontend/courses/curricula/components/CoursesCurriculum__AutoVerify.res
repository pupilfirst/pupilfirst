...
let completeButtonText = (title, iconClasses) =>
  <span> <FaIcon classes={iconClasses ++ " ltr:mr-2 rtl:ml-2"} /> {title |> str} </span>
...
let previewLinkToComplete = link =>
  <a
    ...
    <span> <FaIcon classes="fas fa-external-link-alt ltr:mr-2 rtl:ml-2" /> {tr("visit_link") |> str} </span>
  </a>
...
let statusBar = (string, linkToComplete) => {
  ...
  let message =
    <div className="flex items-center">
      <span className="fa-stack text-lg ltr:mr-1 rtl:ml-1 text-green-500">
  ...
  let visitLink = link =>
    <a className="text-right w-full" href=link target="_blank">
      <i className="fas fa-external-link-alt ltr:mr-2 rtl:ml-2" /> {tr("visit_link") |> str}
    </a>
