...
describe Courses::CreateLeaderboardEntriesService do
  ...
  before do
    ...
    complete_target(team_2.founders.first, target_3, passed_at: passed_at)
  ...
  describe '#execute' do
    it 'does something' do
      expected_entry_count = team_1.founders.count + team_2.founders.count
      ...
      expect(
        LeaderboardEntry
          .joins(:founder)
          .where(founders: { id: team_3.founders })
          .count
      ).to eq(0)
      ...
      expect(
        LeaderboardEntry
          .joins(:founder)
          .where(founders: { id: team_4.founders })
          .count
      ).to eq(0)
      ...
      expect(
        LeaderboardEntry
          .joins(:founder)
          .where(founders: { id: team_1.founders })
          .pluck(:score)
      ).to eq([3] * team_1.founders.count)
      expect(
        LeaderboardEntry
          .joins(:founder)
          .where(founders: { id: team_2.founders })
          .pluck(:score)
      ).to eq([5] * team_2.founders.count)
    end
  end
end
