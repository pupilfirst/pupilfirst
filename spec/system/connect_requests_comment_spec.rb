...
feature 'Connect Request Comment' do
  ...
  let(:connect_request) do
    create :connect_request, connect_slot: connect_slot, startup: startup
  end
  ...
  scenario 'Faculty visits comment submit page' do
    visit connect_request_feedback_from_faculty_url(
            id: connect_request.id,
            token: faculty.token,
            rating: rating
          )
    expect(page).to have_text(
      'Do you have any additional feedback for SV.CO team?'
    )
  ...
  scenario 'Founder visits comment submit page' do
    visit connect_request_feedback_from_team_url(
            id: connect_request.id,
            token: founder.auth_token,
            rating: rating
          )
    expect(page).to have_text(
      'Do you have any additional feedback for SV.CO team?'
