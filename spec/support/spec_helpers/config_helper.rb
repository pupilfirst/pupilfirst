    ...
    original_secrets_values =
      secrets
        ...
        .each_with_object({}) do |key, original_values|
          original_values[key] = Rails.application.credentials.send(key.to_s)
          Rails.application.credentials.send(key.to_s + '=', secrets[key])
        end

    yield

    original_secrets_values.each do |key, value|
      Rails.application.credentials.send(key.to_s + '=', value)
    end
  end
end
