module TimelineEvents
  class ReviewDataService
    ...
    def data
      TimelineEvent.pending.includes({ founder: :user }, { startup: :level }, { target: { target_skills: :skill } }, :timeline_event_files).order('timeline_events.created_at').each_with_object({}) do |event, hash|
        hash[event.id] = {
          event_id: event.id,
          title: event.title,
