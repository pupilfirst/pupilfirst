...
describe 'leaderboard_entries:create' do
  ...
  before do
    ...
    allow(Courses::CreateLeaderboardEntriesService).to receive(:new)
      .with(enabled_course)
      .and_return(cles)
  end
  ...
  it 'invokes Courses::CreateLeaderboardEntriesService for leaderboard-enabled courses and ignores disabled ones' do
    expect(Courses::CreateLeaderboardEntriesService).not_to receive(:new).with(
      disabled_course
    )
