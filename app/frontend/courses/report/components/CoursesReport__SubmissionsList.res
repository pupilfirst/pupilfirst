...
let unselected = selectedStatus => {
  let unselectedStatus = Array.map(
    Selectable.targetStatus,
    Js.Array.filter(
      status =>
...
let submissionsSorter = (sortDirection, updateSortDirectionCB) => {
  ...
  <div ariaLabel="Change submissions sorting" className="shrink-0 mt-3 md:mt-0 md:ms-2">
    <label className="block text-tiny font-semibold uppercase"> {str(tr("sort_by"))} </label>
    <SubmissionsSorter
      criteria
...
let showSubmission = (submissions, teamStudentIds) => <div> {React.array(Array.map(submission => {
        let teamMismatch = switch Submission.targetRole(submission) {
        | Student => false
        ...
        <div className="" key={Submission.id(submission)}>
          <a
            className="block relative rounded-lg focus:outline-none focus:ring focus-ring-inset focus:ring-focusColor-500"
            ariaLabel={"Student submission " ++ Submission.id(submission)}
            href=submissionHref>
            <div key={Submission.id(submission)} className={submissionCardClasses(submission)}>
              <div className="w-full md:w-3/4">
                <div className="block text-sm md:pe-2">
                  <span className="ms-1 font-semibold text-base">
                    {(Belt.Option.mapWithDefault(
                      Submission.milestoneNumber(submission),
                      "",
                ...
                <div className="mt-1 ms-px text-xs text-gray-900">
                  <span className="ms-1">
                    {str(
                      tr(
                        ~variables=[("date", Submission.createdAtPretty(submission))],
                        "submitted_on",
                      ),
                    )}
                  </span>
            ...
            ? <div
                ...
                <div className="flex flex-1 justify-start items-center pe-8">
                  ...
                  <div className="inline-block ps-3 ">
                    {str(tr("submission_not_considered"))}
                ...
                <a
                  ...
                  <span className="hidden md:inline"> {str(tr("view"))} </span>
                  {str(ts("target"))}
...
let make = (
  ...
  React.useEffect3(() => {
    if Submissions.needsReloading(selectedStatus, sortDirection, submissions) {
      send(BeginReloading)
