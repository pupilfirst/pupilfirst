...
let headerSectiom = (userName, preferredName, userTitle, avatarUrl, showUserEdit) => {
  ...
  <div className="max-w-4xl mx-auto pt-12 flex items-center justify-between px-3 lg:px-0">
    <div className="flex">
      ...
      | Some(src) =>
        <img
          className="w-16 h-16 rounded-full border object-cover border-gray-300 overflow-hidden shrink-0 ltr:mr-4 rtl:ml-4"
          src
        />
      | None =>
        <Avatar
          name
          className="w-16 h-16 ltr:mr-4 rtl:ml-4 border border-gray-300 rounded-full overflow-hidden shrink-0"
        />
      }}
    ...
    {ReactUtils.nullUnless(
      <a className="btn" href="/user/edit">
        <i className="fas fa-edit text-xs md:text-sm ltr:mr-2 rtl:ml-2" />
...
let navButtonClasses = selected =>
  "font-semibold border-b-2 text-sm py-4 ltr:mr-6 rtl:ml-6 hover:text-primary-500 hover:border-gray-300 focus:border-gray-300 focus:text-primary-500 focus:outline-none " ++ (
  ...
  <div className="border-b mt-6">
    <div role="tablist" className="flex max-w-4xl mx-auto px-3 lg:px-0">
      <button
        ...
        <i className="fas fa-book text-xs md:text-sm ltr:mr-2 rtl:ml-2" /> <span> {t("my_courses")->str} </span>
      </button>
      {ReactUtils.nullUnless(
        <button
          role="tab"
          ...
          <i className="fas fa-users text-xs md:text-sm ltr:mr-2 rtl:ml-2" />
      ...
      {ReactUtils.nullUnless(
        <button
          ...
          <i className="fas fa-certificate text-xs md:text-sm ltr:mr-2 rtl:ml-2" />
...
let courseLink = (href, title, icon) =>
  <a
    key=href
    href
    className="px-2 py-1 ltr:mr-2 rtl:ml-2 mt-2 rounded text-sm bg-gray-50 text-gray-500 hover:bg-gray-50 hover:text-primary-500 focus:outline-none focus:bg-gray-50 focus:text-primary-500">
    <i className=icon /> <span className="font-medium ltr:ml-2 rtl:mr-2"> {title->str} </span>
  </a>
...
let ctaButton = (title, href) =>
  <a
    ...
    <span> <i className="fas fa-book" /> <span className="ltr:ml-2 rtl:mr-2"> {title->str} </span> </span>
...
let ctaText = (message, icon) =>
  <div
    ...
    <span> <i className=icon /> <span className="ltr:ml-2 rtl:mr-2"> {message->str} </span> </span>
  </div>
...
let communityLinks = (communityIds, communities) => Js.Array.map(id => {
    ...
    | Some(c) =>
      <a
        ...
        <span className="font-medium ltr:ml-2 rtl:mr-2"> {Community.name(c)->str} </span>
      </a>
    | None => React.null
    }
            ...
            <div>
              <div className="relative">
                ...
                <div
                  ...
                  key={course->Course.id}>
                  <h4
                    className="user-dashboard-course__title text-white font-semibold leading-tight ltr:pl-6 rtl:pr-6 ltr:pr-4 rtl:pl-4 text-lg md:text-xl">
