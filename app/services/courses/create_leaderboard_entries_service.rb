module Courses
  class CreateLeaderboardEntriesService
    ...
    def execute(period_from, period_to)
      ...
      log(
        "Recording leaderboard entries for #{founders.count} students in Course##{@course.id}..."
      )

      LeaderboardEntry.transaction do
        entries =
          founders.each_with_object([]) do |founder, leaderboard_entries|
            score =
              founder
                .timeline_events
                .where(passed_at: [period_from..period_to])
                .sum do |timeline_event|
