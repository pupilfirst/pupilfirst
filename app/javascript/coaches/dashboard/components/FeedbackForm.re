...
let handleResponseJSON = (state, send, te, replaceTimelineEvent, json) =>
  ...
  | None =>
    Notification.success("Feedback Sent", "Your feedback has been recorded and emailed to the student(s)");
    te |> TimelineEvent.updateFeedback(state.feedbackHTML) |> replaceTimelineEvent;
    clearFeedback(send, ());
  };

let sendFeedback = (state, send, te, replaceTimelineEvent, authenticityToken, _event) => {
  ...
  Js.Promise.(
    ...
    |> then_(json => json |> handleResponseJSON(state, send, te, replaceTimelineEvent) |> resolve)
    |> catch(error =>
         (
