    ...
    def generate_url
      signer =
        Aws::CloudFront::UrlSigner.new(
          key_pair_id: Rails.application.credentials.cloudfront[:key_pair_id],
          private_key:
            Base64.decode64(Rails.application.credentials.cloudfront[:private_key])
        )

      uri =
        URI(
          "https://#{Rails.application.credentials.cloudfront[:host]}/#{@blob.key}"
        )
      ...
      signer.signed_url(
        uri.to_s,
        expires:
          Time.zone.now + Rails.application.credentials.cloudfront[:expiry].seconds
      )
    end
  end
end
