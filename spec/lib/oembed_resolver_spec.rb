  ...
  describe '#embed_code' do
    context 'when supplied a YouTube URL' do
      let(:expected_embed_code) do
        "\u003ciframe width=\"480\" height=\"270\" src=\"https:\/\/www.youtube.com\/embed\/3QDYbQIS8cQ?feature=oembed\" frameborder=\"0\" allow=\"accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture\" allowfullscreen\u003e\u003c\/iframe\u003e"
      end
      ...
      it 'returns embed code for YouTube' do
        expect(
          subject.new('https://www.youtube.com/watch?v=3QDYbQIS8cQ').embed_code
        ).to eq(expected_embed_code)
        expect(subject.new('https://youtu.be/3QDYbQIS8cQ').embed_code).to eq(
          expected_embed_code
        )
      end
    end
    ...
    context 'when supplied a Vimeo URL' do
      let(:expected_embed_code) do
        "<iframe src=\"https://player.vimeo.com/video/15298502?app_id=122963\" width=\"480\" height=\"270\" frameborder=\"0\" title=\"Happy Punisher\" allow=\"autoplay; fullscreen\" allowfullscreen></iframe>"
      end

      before do
        stub_request(
          :get,
          'https://vimeo.com/api/oembed.json?url=https://vimeo.com/15298502'
        ).to_return(
          body:
            '{"type":"video","version":"1.0","provider_name":"Vimeo","provider_url":"https:\/\/vimeo.com\/","title":"Happy Punisher","author_name":"Ryan Shelley","author_url":"https:\/\/vimeo.com\/barnapkins","is_plus":"1","account_type":"plus","html":"<iframe src=\"https:\/\/player.vimeo.com\/video\/15298502?app_id=122963\" width=\"480\" height=\"270\" frameborder=\"0\" title=\"Happy Punisher\" allow=\"autoplay; fullscreen\" allowfullscreen><\/iframe>","width":480,"height":270,"duration":103,"description":"I find this video relaxing.","thumbnail_url":"https:\/\/i.vimeocdn.com\/video\/91917335_295x166.webp","thumbnail_width":295,"thumbnail_height":166,"thumbnail_url_with_play_button":"https:\/\/i.vimeocdn.com\/filter\/overlay?src0=https%3A%2F%2Fi.vimeocdn.com%2Fvideo%2F91917335_295x166.webp&src1=http%3A%2F%2Ff.vimeocdn.com%2Fp%2Fimages%2Fcrawler_play.png","upload_date":"2010-09-26 11:34:00","video_id":15298502,"uri":"\/videos\/15298502"}',
          status: 200
        )
      end

      it 'returns embed code for Vimeo' do
        expect(subject.new('https://vimeo.com/15298502').embed_code).to eq(
          expected_embed_code
        )
      end
    end
    ...
    context 'when supplied a Slideshare URL' do
      ...
      it 'returns embed code for Slideshare' do
        expect(
          subject.new(
            'https://www.slideshare.net/erickjones014/amazing-fact-about-cats-72889434'
          ).embed_code
        ).to eq(expected_embed_code)
      end
    end
    ...
    context 'when supplied a Google Slides URL' do
      let(:pasted_link) do
        'https://docs.google.com/presentation/d/e/SLIDES-CODE/pub?start=false&loop=false&delayms=3000'
      end
      let(:expected_embed_code) do
        "<iframe src='https://docs.google.com/presentation/d/e/SLIDES-CODE/embed?start=false&loop=false&delayms=3000' frameborder='0' width='960' height='572' allowfullscreen='true' mozallowfullscreen='true' webkitallowfullscreen='true'></iframe>"
      end
    ...
    context 'when supplied a Google Forms URL' do
      let(:pasted_link) do
        'https://docs.google.com/forms/d/e/FORMS-CODE/viewform?usp=sf_link'
      end
      let(:expected_embed_code) do
    ...
    context 'when supplied a Google Spreadsheets URL' do
      let(:pasted_link) do
        'https://docs.google.com/spreadsheets/d/e/SPREADSHEET-CODE/pubhtml'
      end
      let(:expected_embed_code) do
    ...
    context 'when supplied an unknown URL' do
      it 'raises Oembed::Resolver::ProviderNotSupported' do
        expected_error_message =
          "The hostname 'speakerdeck.com' could not be resolved to any known provider."

        expect do
          subject.new('https://speakerdeck.com/hiroki6/monad-error-with-cats')
            .embed_code
        end.to(
          raise_error(
            Oembed::Resolver::ProviderNotSupported,
            expected_error_message
          )
        )
      end
