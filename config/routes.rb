Rails
  ...
  .draw do
    ...
    devise_scope :user do
      post 'users/send_login_email',
           controller: 'users/sessions',
           action: 'send_login_email',
           as: 'user_send_login_email'
      ...
      if Rails.env.development?
        get 'users/auth/developer',
            controller: 'users/omniauth_callbacks',
            action: 'passthru',
            as: 'user_developer_omniauth_authorize'
        post 'users/auth/developer/callback',
             controller: 'users/omniauth_callbacks',
             action: 'developer'
      end
    end
    ...
    post 'users/email_bounce',
         controller: 'users/postmark_webhook',
         action: 'email_bounce'
    ...
    namespace :school, module: 'schools' do
      resources :faculty,
                ...
                as: 'coaches',
                path: 'coaches' do
        collection { get '/', action: 'school_index' }
      end
      ...
      resources :courses, only: %i[index show new] do
        ...
        resources :faculty, as: 'coaches', path: 'coaches', only: [] do
          collection { get '/', action: 'course_index' }
        end
    ...
    resources :faculty, only: %i[] do
      collection do
        ...
        get 'weekly_slots/:token',
            to: 'faculty#weekly_slots',
            as: 'weekly_slots'
        post 'save_weekly_slots/:token',
             to: 'faculty#save_weekly_slots',
             as: 'save_weekly_slots'
        delete 'weekly_slots/:token',
               to: 'faculty#mark_unavailable',
               as: 'mark_unavailable'
    ...
    scope 'connect_request',
          controller: 'connect_request',
          as: 'connect_request' do
      get ':id/feedback/from_team/:token',
          action: 'feedback_from_team',
          as: 'feedback_from_team'
      get ':id/feedback/from_faculty/:token',
          action: 'feedback_from_faculty',
          as: 'feedback_from_faculty'
      get ':id/join_session(/:token)',
          action: 'join_session',
          as: 'join_session'
      patch ':id/feedback/comment/:token',
    ...
    get 'styleguide',
        to: 'home#styleguide',
        constraints: DevelopmentConstraint.new
    ...
    get 'agreements/terms-of-use',
        to: redirect('/agreements/terms-and-conditions')
    ...
    resources :timeline_events, only: %i[show], path: 'submissions' do
      member { get 'review', action: 'review' }
    end
    ...
    get '/c/:serial_number',
        to: 'issued_certificates#verify',
        as: :issued_certificate
    get '/help/:document', to: 'help#show'
    get '/oauth/:provider',
        to: 'home#oauth',
        as: 'oauth',
        constraints: SsoConstraint.new
