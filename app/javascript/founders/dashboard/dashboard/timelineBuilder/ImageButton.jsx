...
export default class ImageButton extends React.Component {
  ...
  validate(inputElement) {
    ...
    if (
      $.inArray(fileExtension.toLowerCase(), ["png", "jpg", "jpeg", "svg"]) ==
      -1
    ) {
  ...
  render() {
    return (
      <div
        className={this.actionTabClasses()}
        onClick={this.handleImageButtonClick}
        data-toggle="popover"
        data-title="File Invalid!"
        data-content={this.errorPopoverText()}
        data-placement="bottom"
        data-trigger="manual"
      >
        <label className="sr-only" htmlFor="timeline-builder__image-input">
          Cover Image
        </label>
        <input
          id="timeline-builder__image-input"
          type="file"
