...
describe Courses::CreateLeaderboardEntriesService do
  ...
  let(:startup_4) do
    create :startup, level: level_1, name: 's4', access_ends_at: 1.day.ago
  end
  ...
  describe '#execute' do
    it 'does something' do
      ...
      expect(
        LeaderboardEntry
          .joins(:founder)
          .where(founders: { id: startup_3.founders })
          .count
      ).to eq(0)
      ...
      expect(
        LeaderboardEntry
          .joins(:founder)
          .where(founders: { id: startup_4.founders })
          .count
      ).to eq(0)
      ...
      expect(
        LeaderboardEntry
          .joins(:founder)
          .where(founders: { id: startup_1.founders })
          .pluck(:score)
      ).to eq([3] * startup_1.founders.count)
      expect(
        LeaderboardEntry
          .joins(:founder)
          .where(founders: { id: startup_2.founders })
          .pluck(:score)
      ).to eq([5] * startup_2.founders.count)
    end
  end
end
