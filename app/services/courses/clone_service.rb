module Courses
  ...
  class CloneService
    ...
    def clone(new_name, school)
      Course.transaction do
        Course
          ...
          .tap do |new_course|
            evaluation_criteria_translation =
              create_evaluation_criteria(new_course)
            levels = create_levels(new_course)
            target_groups = create_target_groups(levels)
            targets =
              create_targets(target_groups, evaluation_criteria_translation)
    ...
    def create_evaluation_criteria(new_course)
      @course
        .evaluation_criteria
        .each_with_object({}) do |old_ec, translation|
          new_ec =
            new_course.evaluation_criteria.create!(
              name: old_ec.name,
        ...
        old_target_group
          ...
          .map do |old_target|
            ...
            if old_target.evaluation_criteria.exists?
              create_target_evaluation_criteria(
                old_target,
                new_target,
                evaluation_criteria_translation
              )
    ...
    def create_target_evaluation_criteria(
      old_target,
      new_target,
      evaluation_criteria_translation
    )
      old_target.target_evaluation_criteria.each do |t_ec|
        new_target.target_evaluation_criteria.create!(
          evaluation_criterion_id:
            evaluation_criteria_translation[t_ec.evaluation_criterion_id]
        )
      end
    end
