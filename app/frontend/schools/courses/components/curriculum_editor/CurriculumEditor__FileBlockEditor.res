...
let onChange = (contentBlock, updateContentBlockCB, event) => {
  ...
  let newContentBlock = ContentBlock.updateFile(newTitle, contentBlock)
  updateContentBlockCB(newContentBlock)
}

@react.component
let make = (~url, ~title, ~filename, ~contentBlock, ~updateContentBlockCB) => {
  let titleInputId = "title-" ++ ContentBlock.id(contentBlock)
  ...
  <div className="relative border border-gray-300 rounded-lg">
        ...
        <a
          ...
          <div className="flex items-center">
            ...
            <div className="ps-4 leading-tight h-12 flex flex-col justify-center">
              <div className="text-lg font-semibold"> {str(title)} </div>
              <div className="text-sm italic text-gray-600"> {str(filename)} </div>
            </div>
          </div>
        </a>
      </div>
    ...
    <div className="flex border-t justify-end">
      <div className="flex-1 content-block__action-bar-input p-3">
        <label htmlFor=titleInputId className="text-sm font-semibold"> {str(t("title"))} </label>
