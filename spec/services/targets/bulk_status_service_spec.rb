  ...
  describe '#status' do
    ...
    context 'when the founder target has a verified submission' do
      it 'returns :complete' do
        founder_event.update!(target: founder_target, verified_status: TimelineEvent::VERIFIED_STATUS_VERIFIED)
    ...
    context 'when the founder target has a needs_improvement submission' do
      it 'returns :needs_improvement' do
        founder_event.update!(target: founder_target, verified_status: TimelineEvent::VERIFIED_STATUS_NEEDS_IMPROVEMENT)
    ...
    context 'when the founder target has a not_accepted submission' do
      it 'returns :not_accepted' do
        founder_event.update!(target: founder_target, verified_status: TimelineEvent::VERIFIED_STATUS_NOT_ACCEPTED)
    ...
    context 'when the founder has multiple submissions for target' do
      it 'returns status based on the latest submission' do
        founder_event.update!(target: founder_target, verified_status: TimelineEvent::VERIFIED_STATUS_NOT_ACCEPTED, created_at: 2.days.ago)
        founder_event_2.update!(target: founder_target, verified_status: TimelineEvent::VERIFIED_STATUS_VERIFIED, created_at: 1.day.ago)
        expect(subject.status(founder_target.id)).to eq(Target::STATUS_COMPLETE)
      end
    end
    ...
    context 'when the co-founder has completed a startup target' do
      it 'returns :complete' do
        co_founder_event.update!(target: startup_target, verified_status: TimelineEvent::VERIFIED_STATUS_VERIFIED)
    ...
    context 'when the startup iteration is different from a chores event iteration' do
      it 'returns status from previous iteration' do
        founder_event.update!(target: founder_chore, verified_status: TimelineEvent::VERIFIED_STATUS_VERIFIED)
        startup.update!(iteration: 2)
