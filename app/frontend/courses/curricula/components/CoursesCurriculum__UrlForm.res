...
let validate = url => {
  let urlLength = String.length(url)
...
let updateUrl = (send, typingCB, event) => {
  let value = Js.String.trim(ReactEvent.Form.target(event)["value"])
  typingCB(String.length(value) > 0)
...
let isDisabled = state =>
  switch state.url {
  | "" => true
  | _someUrl => ListUtils.isNotEmpty(state.errors)
  }
