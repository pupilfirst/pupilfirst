...
export default class FeedbackActions extends React.Component {
  constructor(props) {
    super(props);
  ...
  sendSlackFeedback(event) {
    let that = this;
    let postUrl =
      "/admin/timeline_events/" + this.props.eventId + "/send_slack_feedback";
    ...
    $.post({
      ...
      data: {
        ...
        founder_id: that.props.founderId
      }
    })
      .done(function() {
        new PNotify({
  ...
  sendEmailFeedback() {
    let that = this;
    let postUrl =
      "/admin/timeline_events/" + this.props.eventId + "/send_email_feedback";
