...
RSpec.describe Feature, type: :model do
  ...
  describe '.active?' do
    ...
    context 'when a feature is not configured' do
      it 'returns false' do
        expect(
          Feature.active?(:non_existent_feature, user('someone@sv.co'))
        ).to eq(false)
      end
    end
    ...
    context 'with a regex feature' do
      before do
        create :feature,
               value: { email_regexes: %w[\S+@sv.co$ \S+@mobme.in$] }.to_json
      end

      it 'returns true for emails that match regex' do
        expect(Feature.active?(:test_feature, user('someone@sv.co'))).to eq(
          true
        )
        expect(
          Feature.active?(:test_feature, user('someone_else@mobme.in'))
        ).to eq(true)
      end

      it 'returns false for emails that do not match regex' do
        expect(Feature.active?(:test_feature, user('someone@sv.com'))).to eq(
          false
        )
    ...
    context 'with a email-list based feature' do
      before do
        create :feature,
               value: { emails: %w[someone@sv.co someone@mobme.in] }.to_json
      end
      ...
      it 'returns true for emails that are in the list' do
        ...
        expect(Feature.active?(:test_feature, user('someone@mobme.in'))).to eq(
          true
        )
      end
      ...
      it 'returns false for emails not in the list' do
        expect(
          Feature.active?(:test_feature, user('someone_else@sv.co'))
        ).to eq(false)
      end
    end
    ...
    context 'with an activated feature' do
      ...
      it 'returns true for everyone' do
        ...
        expect(Feature.active?(:test_feature, user('foo@example.com'))).to eq(
          true
        )
      end
    end
