module Users
  class InactivityNotificationAndDeletionService
    ...
    def applicable_schools
      if Settings.delete_inactive_users_after.positive?
        School.all
      else
    ...
    def users_to_notify_deletion
      @users_to_notify_deletion ||=
        applicable_schools.map do |school|
    ...
    def login_url(school)
      ...
      url_options = { host: host.fqdn, protocol: 'https' }
    ...
    def deletion_period(school)
      Schools::Configuration.new(school).delete_inactive_users_after.presence ||
        Settings.delete_inactive_users_after
    end
  end
end
