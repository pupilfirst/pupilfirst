      ...
      <div className="border-b px-6 py-2 items-center justify-between">
        <div className="inline-block relative w-64">
          <select
            ...
            {levels
             |> List.map(level =>
                  <option key={level |> Level.number |> string_of_int} value={level |> Level.number |> string_of_int}>
      ...
      <div className="bg-grey-darker px-3">
            ...
            <label className="flex items-center leading-tight mr-4 my-auto">
              ...
              <span id="selected-students" className="ml-2 text-sm text-grey-dark">
                {let selectedCount = state.selectedStudents |> List.length
                 let studentCount =
                   filteredTeams(state) |> List.map(team => team |> Team.students) |> List.flatten |> List.length
        ...
        <div
          ...
          <div className="flex items-center justify-between">
            ...
            />
            <div onClick={_e => send(ToggleFilterVisibility)} className="flex text-indigo items-center">
                                  ...
                                  <div className="flex items-center">
                                    <label className="block text-grey leading-tight font-bold px-4 py-5">
                                      <input
                                        ...
                                        onChange={
                                          isChecked ?
                                            _e => send(DeselectStudent(student)) :
                                            (_e => send(SelectStudent(student)))
                                        }
                                      />
                                    </label>
                                    <div
