...
class Founder < ApplicationRecord
  ...
  def timeline_events_for_display(viewer)
    ...
    # Only display verified of needs-improvement events if 'viewer' is not the founder.
    if viewer != self
      events_for_display = events_for_display.verified_or_needs_improvement
    end
    events_for_display.includes(:target, :timeline_event_files).order(:event_on, :updated_at).reverse_order.decorate
  end
end
