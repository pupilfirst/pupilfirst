...
feature 'Faculty Weekly Slots', broken: true do
  let!(:faculty) do
    create :faculty,
           current_commitment:
             '20 mins per week for the first 6 months this year',
           school: school
  end
  ...
  context 'User visits weekly_slot page of faculty with connection slots',
          js: true do
    let!(:connection_slot_1) do
      create :connect_slot,
             faculty: faculty,
             slot_at: ConnectSlot.next_week_start + 7.hours
    end
    let!(:connection_slot_2) do
      create :connect_slot,
             faculty: faculty,
             slot_at: ConnectSlot.next_week_start + 32.hours
    end
    ...
    scenario 'User verifies present slots and commitment' do
      ...
      expect(page).to have_selector(
        '.weekly-slots__connect-slot--selected',
        count: 1,
        text: '07:00'
      )
    ...
    scenario 'User removes a present slot' do
      ...
      first_slot =
        page.find('.weekly-slots__connect-slot--selected', match: :first)
      expect(first_slot).to have_text('07:00')
      first_slot.click
      expect(first_slot[:class]).to_not include(
                                          'weekly-slots__connect-slot--selected'
                                        )
      click_on 'Save'
      expect(page).to have_text(
        'We have successfully recorded your availability for the upcoming week'
      )
    ...
    scenario 'User add two new slots' do
      ...
      expect(
        page.find(
          '.weekly-slots__connect-slot[data-day="1"][data-time="11.5"]'
        )[
          :class
        ]
      ).to_not include('weekly-slots__connect-slot--selected')
      page.find('.weekly-slots__connect-slot[data-day="1"][data-time="11.5"]')
        .click
      expect(
        page.find(
          '.weekly-slots__connect-slot[data-day="1"][data-time="11.5"]'
      ...
      ).to include('weekly-slots__connect-slot--selected')
      ...
      expect(
        page.find(
          '.weekly-slots__connect-slot[data-day="2"][data-time="22.0"]'
        )[
          :class
        ]
      ).to_not include('weekly-slots__connect-slot--selected')
      page.find('.weekly-slots__connect-slot[data-day="2"][data-time="22.0"]')
        .click
      expect(
        page.find(
          '.weekly-slots__connect-slot[data-day="2"][data-time="22.0"]'
      ...
      expect(
        faculty.connect_slots.find_by(
          slot_at: ConnectSlot.next_week_start + 11.hours + 30.minutes
        )
      ).to be_present
      expect(
        faculty.connect_slots.find_by(
          slot_at: ConnectSlot.next_week_start + 46.hours
        )
      ).to be_present
    end
