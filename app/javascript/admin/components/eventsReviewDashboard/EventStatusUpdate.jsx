...
import EventSkillGrading from "./EventSkillGrading";

export default class EventStatusUpdate extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      status: null,
  ...
  saveReview(event) {
    ...
    if (!this.state.status) {
      ...
      (!this.state.grade && !this.state.points)
    ) {
        ...
        error: function(response) {
          let error =
            response.responseJSON && response.responseJSON.error
              ...
              : "Something went wrong at the server. Try again";
          alert(error);
  ...
  alreadyReviewed() {
    return (
      this.props.rootState.reviewData[this.props.eventId].reviewed === true
    );
  }
  ...
  enableSaveReviewButton() {
    ...
    return (
      (!this.rubricPresent() &&
        this.state.status === "verified" &&
        (this.state.grade || this.state.points)) ||
      this.state.status === "needs_improvement" ||
    ...
    return (
      <div className="margin-bottom-10">
        ...
        <div onChange={this.changeStatus}>
          <label
            className="review-dashboard-event-status-update__label"
            htmlFor={this.radioInputId("verified")}
          >
            <input
              className="review-dashboard-event-status-update__input"
              type="radio"
              id={this.radioInputId("verified")}
              value="verified"
          ...
          <label
            className="review-dashboard-event-status-update__label"
            htmlFor={this.radioInputId("needs_improvement")}
          >
            <input
              className="review-dashboard-event-status-update__input"
              type="radio"
              id={this.radioInputId("needs_improvement")}
          ...
          <label
            className="review-dashboard-event-status-update__label"
            htmlFor={this.radioInputId("not_accepted")}
          >
            <input
              className="review-dashboard-event-status-update__input"
              type="radio"
              id={this.radioInputId("not_accepted")}
              value="not_accepted"
              name={this.radioInputName("status")}
            />
            Not Accepted
              ...
              {this.props.targetId !== "null" && (
                <div>
                  ...
                  <div onChange={this.changeGrade}>
                    <label
                      className="review-dashboard-event-status-update__label"
                      htmlFor={this.radioInputId("wow")}
                    >
                      <input
                        className="review-dashboard-event-status-update__input"
                        type="radio"
                        id={this.radioInputId("wow")}
                        value="wow"
                        name={this.radioInputName("grade")}
                      />
                      Wow
                    </label>
                    <label
                      className="review-dashboard-event-status-update__label"
                      htmlFor={this.radioInputId("great")}
                    >
                      <input
                        className="review-dashboard-event-status-update__input"
                        type="radio"
                        id={this.radioInputId("great")}
                        value="great"
                        name={this.radioInputName("grade")}
                      />
                      Great
                    ...
                    <label
                      className="review-dashboard-event-status-update__label"
                      htmlFor={this.radioInputId("good")}
                    >
                      <input
                        className="review-dashboard-event-status-update__input"
                        type="radio"
                        id={this.radioInputId("good")}
                        value="good"
                        name={this.radioInputName("grade")}
                      />
                      Good
        ...
        {this.enableSaveReviewButton() && (
          <div>
            {!this.alreadyReviewed() && (
              <div>
                <a
                  className="button cursor-pointer margin-bottom-10"
        ...
        {this.state.status === "verified" &&
          this.rubricPresent() && (
            <EventSkillGrading
              ...
              rubric={
                this.props.rootState.reviewData[this.props.eventId].rubric
              }
            />
          )}
