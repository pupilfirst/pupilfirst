class Statistic < ActiveRecord::Base
  ...
  def self.chartkick_parameter_by_date(parameter,incubation_location: nil, from: 30.days.ago, to: Time.now)
    where(parameter: parameter, incubation_location: incubation_location, created_at: from..to).order('id DESC').reverse.inject({}) do |stats, statistic|
