class UserMailer < SchoolMailer
  ...
  def new_post(post, addressee)
    ...
    simple_mail(addressee.email, I18n.t("mailers.user.new_post_subject"))
  end
  ...
  def daily_digest(user, updates)
    ...
    @school = user.school
    subject =
      "#{user.school.name}: #{I18n.t("mailers.user.daily_digest_subject")} - #{Time.zone.now.strftime('%b %-d, %Y')}"
  ...
  def delete_account_token(user, delete_account_url)
    ...
    simple_mail(user.email, I18n.t("mailers.user.delete_account_token_subject", school_name: @school.name))
  end
  ...
  def confirm_account_deletion(name, email, school)
    ...
    @email = email
    @school = school
    simple_mail(email, I18n.t("mailers.user.confirm_account_deletion_subject", school_name: @school.name))
  end
  ...
  def account_deletion_notification(user, sign_in_url, inactivity_months)
    ...
    simple_mail(
      user.email,
      I18n.t("mailers.user.account_deletion_notification_subject")
    )
  end
end
