...
displayContactAlerts = ->
  if $("#contact_form_query_type").val() == "Space Availability Question"
    ...
    $("#contact-alerts").html('<p><strong>Note:</strong> Our admission process is in two batches per year. Because of the volume of requests we receive, we will not be able to help founders outside our well-defined 6-month program. If you meet the eligibility criteria please keep a tab on our blog and our website to get to know when the applications for the next batch open.</p><p>Also please note that our batches are focused on one particular domain (e.g. Batch 1: FinTech, Batch 2: SaaS) and we will not be providing case by case assistance to startups.</p>')
    $("#contact-alerts").show()
  else if $("#contact_form_query_type").val() == "Startup India Recommendation Query"
    $("#contact-alerts").html("<p><strong>Note:</strong> Startup Village now recommends startups who qualify for India's Startup India program. To know details of how to apply, <a href='https://blog.sv.co/recommendations-for-startup-india-6ef14251649a' target='_blank'>read this blog post</a>.</p>")
    $("#contact-alerts").show()
  else
    $("#contact-alerts").hide()
