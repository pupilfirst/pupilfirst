  ...
  class LeaderboardPresenter < ApplicationPresenter
    ...
    def heading
      if current_user_is_topper?
        return(
          '<span class="font-bold">You</span> are at the top of the leaderboard. <span class="font-bold">Congratulations!</span>'
            .html_safe
        )
      end

      multiple_mid_text =
        'are at the top of the leaderboard this week, sharing a score of '
    ...
    def rank_change_icon(delta)
      if delta >= 10
        ...
        view.image_tag(
          ...
          alt: 'Rank change down'
        )
      else
        view.image_tag(
          'courses/leaderboard/rank-change-down-double.svg',
          alt: 'Rank change down double'
        )
      end
    end
