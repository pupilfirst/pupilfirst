  ...
  class CreatePlatformTestAccountService
    ...
    def execute
      User.transaction do
        # Create the user.
        user =
          User.where(email: @email, school: @startup.school).first_or_create!
        user.update!(name: @name, title: 'Test Account')

        Rails.logger.info(
          "User with email address '#{@email}' created: ##{user.id}."
        )
        ...
        Rails.logger.info(
          "Founder for user created: ##{founder.id}, and added to Startup##{@startup.id}."
        )
        ...
        Rails.logger.info(
          "Faculty for user created: ##{faculty.id}), and added as coach for Startup##{@startup.id}."
        )
      end
    end
  end
