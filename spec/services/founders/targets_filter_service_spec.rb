...
describe Founders::TargetsFilterService do
  ...
  let(:startup_2) { create :startup, batch: batch }
  ...
  let!(:expiring_target) { create :target, :for_startup, days_to_complete: 5, target_group: target_group }
  let!(:expired_target) { create :target, :for_startup, days_to_complete: 0, target_group: target_group }
  let!(:needs_improvement_target) { create :target, :for_startup, days_to_complete: 20, target_group: target_group }
  let!(:not_accepted_target) { create :target, :for_startup, days_to_complete: 20, target_group: target_group }
  let!(:startup_2_completed_target) { create :target, :for_startup, days_to_complete: 5, target_group: target_group }
  let!(:cofounder_completed_target) { create :target, :for_startup, days_to_complete: 5, target_group: target_group }
  ...
  describe '#filter' do
    ...
    it 'returns targets based on the target filter specified' do
      expect(subject.filter('expires_soon').count).to eq(2)
