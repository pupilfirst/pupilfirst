...
let targetClasses = (target, targets) =>
  ...
  switch (Js.Array.length(targets) == 1, Target.visibility(target)) {
...
let updateSortIndex = (targets, target, up, updateTargetSortIndexCB) => {
  ...
  let targetIds = Js.Array.map(Target.id, newTargets)
...
let editorLink = (linkPrefix, linkSuffix, target, iconClass) => {
  ...
  <Link
    ...
    " " ++
    (linkSuffix ++
    (" " ++ t("of_target") ++ " " ++ Target.title(target)))}
    ariaLabel={t("edit") ++
    " " ++
    (linkSuffix ++
    (" " ++ t("of_target") ++ " " ++ Target.title(target)))}
...
let make = (~target, ~targets, ~updateTargetSortIndexCB, ~index, ~course) => {
  let linkPrefix =
    "/school/courses/" ++ (Course.id(course) ++ ("/targets/" ++ (Target.id(target) ++ "/")))
  ...
  <div
    ...
    {Js.Array.length(targets) > 1
      ? <div
          ...
          <button
            title={t("move_up")}
            ariaLabel={t("move_up") ++ Target.title(target)}
            id={"target-move-up-" ++ Target.id(target)}
          ...
          <button
            title={t("move_down")}
            ariaLabel={t("move_down") ++ Target.title(target)}
            id={"target-move-down-" ++ Target.id(target)}
      ...
      : React.null}
    <Link
      id={"target-show-" ++ Target.id(target)}
      title={t("edit_content") ++ " " ++ Target.title(target)}
      ariaLabel={t("edit_content") ++ " " ++ Target.title(target)}
      className={targetClasses(target, targets)}
      href={linkPrefix ++ "content"}>
      <span className="font-medium text-sm"> {str(Target.title(target))} </span>
      <div className="flex items-center">
        ...
        | Draft =>
          <span
            ...
            <span className="ms-1"> {str(t("draft"))} </span>
          </span>
        | _ => React.null
        }}
        ...
        {Target.hasAssignment(target)
          ? <div
              ...
              <span className="hidden md:block ms-1"> {str(t("assignment_label"))} </span>
            </div>
          : React.null}
      </div>
    </Link>
