module Types
  class TopicType < Types::BaseObject
    ...
    def creator
      BatchLoader::GraphQL
        .for(object.id)
        .batch do |topic_ids, loader|
          Topic
            .includes(first_post: :creator)
            .where(id: topic_ids)
            .each { |topic| loader.call(topic.id, topic.first_post.creator) }
        end
    ...
    def solved
      BatchLoader::GraphQL
        .for(object.id)
        .batch(default_value: false) do |topic_ids, loader|
          Post
            .where(topic_id: topic_ids, solution: true)
            .each { |post| loader.call(post.topic_id, true) }
        end
    ...
    def participants
      BatchLoader::GraphQL
        .for(object.id)
        .batch(default_value: []) do |topic_ids, loader|
          Post
            .includes(:creator)
            .where(topic_id: topic_ids)
            .where('post_number < ?', 4)
            .each do |post|
              loader.call(post.topic_id) do |memo|
                memo |= [post.creator].compact # rubocop:disable Lint/UselessAssignment
              end
            end
        end
      ...
      BatchLoader::GraphQL
        ...
        .batch do |topic_ids, loader|
          Topic
            ...
            .distinct(:creator_id)
            .count(:creator_id)
            .each do |(topic_id, participants_count)|
    ...
    def live_replies_count
      BatchLoader::GraphQL
        ...
        .batch do |topic_ids, loader|
          ...
          scope.each { |topic| loader.call(topic.id, topic.live_replies_count) }
        end
