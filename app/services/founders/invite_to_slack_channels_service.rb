module Founders
  ...
  class InviteToSlackChannelsService
    ...
    def execute
      ...
      public_channels.each do |channel_id|
        invite_to_channel('channels', channel_id)
      end
      log "Inviting Founder ##{@founder.id} to private channels..."
      private_channels.each do |channel_id|
        invite_to_channel('groups', channel_id)
      end
    ...
    def invite_to_channel(channel_type, channel_id)
      api(app_token).get(
        "#{channel_type}.invite",
        params: invite_params(channel_id)
      )
    ...
    def channel_ids(channel_type, secrets_key)
      ...
      channel_hash =
        response[channel_type].each_with_object({}) do |channel, hash|
      ...
      Rails
        ...
        .secrets
        .slack
        .dig(:channels, secrets_key)
        .map { |channel_name| channel_hash[channel_name] }
    end
    ...
    def invite_params(channel_id)
      { channel: channel_id, user: @founder.slack_user_id }
    end
