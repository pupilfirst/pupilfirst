...
feature "Markdown editor", js: true do
  ...
  context "when the user has already attached a lot of files today" do
    around do |example|
      original_value = Rails.application.credentials.max_daily_markdown_attachments
      Rails.application.credentials.max_daily_markdown_attachments = 1

      example.run

      Rails.application.credentials.max_daily_markdown_attachments = original_value
    end
