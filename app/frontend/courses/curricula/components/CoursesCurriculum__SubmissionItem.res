...
let computeId = (index, checklistItem) =>
  string_of_int(index) ++ ("-" ++ ChecklistItem.kindAsString(checklistItem))

let notBlank = string => String.trim(string) != ""

let placeholder = (id, checklistItem) => {
  let title = ChecklistItem.title(checklistItem)
  let optional = ChecklistItem.option(checklistItem)
...
let longTextWarning = value => {
  let currentLength = String.length(value)
...
let attachFile = (updateResultCB, attachingCB, files, id, filename) => {
  attachingCB(false)
  updateResultCB(ChecklistItem.Files(Array.append([ChecklistItem.makeFile(id, filename)], files)))
}
...
let removeFile = (updateResultCB, files, id) =>
  updateResultCB(ChecklistItem.Files(Js.Array.filter(a => ChecklistItem.fileId(a) != id, files)))

let showFiles = (files, preview, id, attachingCB, updateResultCB, index) =>
  <div>
    <div className="flex flex-wrap" id> {React.array(Array.map(file =>
          <div
            key={"file-" ++ ChecklistItem.fileId(file)}
            ariaLabel={"file-" ++ ChecklistItem.filename(file)}
            target="_blank"
            ...
            <div
              ...
              <div className="flex">
                ...
                <span
                  ...
                  {str(ChecklistItem.filename(file))}
                </span>
              </div>
              <button
                title={tr("remove") ++ ChecklistItem.filename(file)}
                ...
                onClick={_ => removeFile(updateResultCB, files, ChecklistItem.fileId(file))}>
                <PfIcon className="if i-times-regular text-sm" />
              </button>
            </div>
          </div>
        , files))} </div>
    {Array.length(files) < 3
