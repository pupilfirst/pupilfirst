...
describe Targets::StatsService do
  ...
  before do
    ...
    (Startup.all - [Startup.first]).each do |startup|
      create(:timeline_event, startup: startup, task: prerequisite_target, verified_status: TimelineEvent::VERIFIED_STATUS_VERIFIED)
    end

    # second startup completed the target
    create(:timeline_event, startup: Startup.second, task: target, verified_status: TimelineEvent::VERIFIED_STATUS_VERIFIED)

    # third startup's submission was rejected
    create(:timeline_event, startup: Startup.third, task: target, verified_status: TimelineEvent::VERIFIED_STATUS_NOT_ACCEPTED)

    # fourth startup's submission was marked needs improvement
    create(:timeline_event, startup: Startup.fourth, task: target, verified_status: TimelineEvent::VERIFIED_STATUS_NEEDS_IMPROVEMENT)

    # fifth startup's submission is pending verification
    create(:timeline_event, startup: Startup.fifth, task: target, verified_status: TimelineEvent::VERIFIED_STATUS_PENDING)
  end
