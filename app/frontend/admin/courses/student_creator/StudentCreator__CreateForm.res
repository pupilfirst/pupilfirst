  ...
  <div className="flex justify-between mb-1">
    ...
    {studentsCount < 2
      ? <span className="text-tiny">
          <i className="fas fa-exclamation-triangle text-orange-600 ltr:mr-1 rtl:ml-1" />
...
let renderTitleAndAffiliation = (title, affiliation) => {
  ...
  | Some(text) =>
    <div className="flex items-center">
      <div className="ltr:mr-1 rtl:ml-1 text-xs text-gray-600"> {str(text)} </div>
    </div>
  | None => React.null
  }
}
...
let tagBoxes = tags =>
  <div className="flex flex-wrap">
    ...
    |> Array.map(tag =>
      <div
        key=tag
        className="flex items-center bg-gray-200 border border-gray-500 rounded-lg px-2 py-px mt-1 ltr:mr-1 rtl:ml-1 text-xs text-gray-900 overflow-hidden">
        {str(tag)}
      </div>
    )
    |> React.array}
...
let studentCard = (studentInfo, send, team, tags) => {
  ...
  <div key={studentInfo |> StudentInfo.email} className=containerClasses>
    <div className="flex flex-col flex-1 flex-wrap p-3">
      <div className="flex items-center">
        <div className="ltr:mr-1 rtl:ml-1 font-semibold"> {StudentInfo.name(studentInfo)->str} </div>
      ...
      <div className="p-4 rounded-lg border bordedr-gray-300 bg-gray-100 mt-1">
        ...
        <div className="mt-4 flex">
          ...
          <label className="checkbox-label flex " htmlFor="notify-new-students">
            ...
            <span className="text-sm rtl:pr-2"> {t("notify_students_label")->str} </span>
          </label>
        </div>
