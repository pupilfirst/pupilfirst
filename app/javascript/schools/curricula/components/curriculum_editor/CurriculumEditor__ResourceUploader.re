...
let component = ReasonReact.reducerComponent("CurriculumEditor__FileUploader");

let resourcesUploadTabClasses = value =>
  value ? "mr-1 resources-upload-tab__link resources-upload-tab__link--active" : "mr-1 resources-upload-tab__link";

let make = (~addResourceCB, _children) => {
  ...component,
  initialState: () => {id: Random.int(99999), resourceType: File, submittable: false, title: ""},
  reducer: (action, state) =>
    switch (action) {
    | UpdateResourceType(resourceType) => ReasonReact.Update({...state, resourceType})
    | UpdateSubmittable(submittable) => ReasonReact.Update({...state, submittable})
  ...
  render: ({state, send}) => {
    ...
    let handleResponseJSON = json => {
      let id = json |> Json.Decode.(field("id", nullable(int))) |> Js.Null.toOption;
      switch (id) {
      | Some(id) => addResourceCB(id, state.title)
      | None => Notification.success("Success", "Target Created")
      };
      let error = json |> Json.Decode.(field("error", nullable(string))) |> Js.Null.toOption;
      switch (error) {
      ...
      <form id=formId onSubmit={event => submitForm(event)}>
        <div
          ...
          {state.resourceType == File ?
             <input name="resource[file]" type_="file" id="file" multiple=false /> :
             <input
               className="appearance-none block w-full bg-white text-grey-darker border border-grey-light rounded py-3 px-4 mb-6 leading-tight focus:outline-none focus:bg-white focus:border-grey"
               name="resource[link]"
               id="link"
