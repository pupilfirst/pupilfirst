...
let make = (~authenticityToken, _children) => {
  ...
  reducer: (action, state) =>
    switch (action) {
    | UpdateEditorAction(editorAction) => ReasonReact.Update({...state, editorAction})
           ...
           |> Js.Array.map(rawCourse => {
                ...
                let gradesAndLabels =
                  ...
                  |> Array.map(gradesAndLabel =>
                       GradesAndLabels.create(gradesAndLabel##grade, gradesAndLabel##label)
                     )
                  |> Array.to_list;
  ...
  render: ({state, send}) => {
    ...
    <div className="flex flex-1 h-screen">
      {switch (state.editorAction) {
       ...
       | ShowForm(course) => <CourseEditor__Form course authenticityToken hideEditorActionCB updateCoursesCB />
       }}
      <div className="flex-1 flex flex-col bg-grey-lightest overflow-hidden">
        <div className="flex px-6 py-2 items-center justify-between overflow-y-scroll">
          <button
        ...
        <div className="px-6 pb-4 mt-5 flex flex-1 bg-grey-lightest overflow-y-scroll">
          <div className="max-w-md w-full mx-auto relative">
            {state.courses
             |> Course.sort
             |> List.map(course =>
                  <div className="flex items-center shadow bg-white rounded-lg overflow-hidden mb-4">
                    <div className="flex w-full" key={course |> Course.id |> string_of_int}>
                      <a
                        ...
                        <div className="text-sm">
                          <span className="text-black font-semibold"> {course |> Course.name |> str} </span>
                        </div>
                      </a>
                      <a
                        href={"/school/courses/" ++ (course |> Course.id |> string_of_int) ++ "/students"}
...
let decode = json => Json.Decode.{authenticityToken: json |> field("authenticityToken", string)};
