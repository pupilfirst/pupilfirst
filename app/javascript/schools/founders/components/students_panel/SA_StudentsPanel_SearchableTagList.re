...
let handleClick = (tag, setSearchString, clickCB, _event) => {
  clickCB(tag);
  setSearchString(_ => "");
};
      ...
      if (allowNewTags
        ...
        [
          <span
            ...
            key=searchString
            onMouseDown={handleClick(searchString, setSearchString, addTagCB)}
    ...
    let searchResults =
      ...
      |> List.map(tag =>
           <span
             ...
             onMouseDown={handleClick(tag, setSearchString, addTagCB)}>
             {tag |> str}
           </span>
         );
...
let make =
  ...
  <div className="mt-2">
          ...
          |> List.map(tag =>
               <div
                 ...
                 <span
                   ...
                   onClick={handleClick(tag, setSearchString, removeTagCB)}>
    ...
    <input
      value=searchString
      onChange={event =>
        setSearchString(_ => ReactEvent.Form.target(event)##value)
      }
    ...
    {if (results |> ListUtils.isNotEmpty) {
       <div
         ...
         {results |> Array.of_list |> React.array}
       </div>;
     } else {
       React.null;
     }}
