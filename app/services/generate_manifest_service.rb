class GenerateManifestService
  ...
  def icon_variant(variant)
    case variant
    when :sm
      icon.variant(combine_options: combine_options('100x100>'))
    when :md
      icon.variant(combine_options: combine_options('144x144>'))
    when :lg
      icon.variant(combine_options: combine_options('192x192>'))
    when :xl3
      icon.variant(combine_options: combine_options('512x512>'))
    else
      icon
    end
  end
