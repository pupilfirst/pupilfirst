...
describe Startups::TeamUpService do
  ...
  let!(:founders_in_same_level) do
    Founder.where(id: [founder_1.id, founder_2.id, founder_3.id, founder_4.id])
  end
  let!(:founders_in_different_levels) do
    Founder.where(id: [founder_5.id, founder_2.id])
  end
  ...
  describe '#team_up' do
    it 'forms a new team with specified founders and updates faculty enrollments' do
      expect { subject.new(founders_in_same_level).team_up(team_name) }.to(
        change { Startup.count }.from(3).to(2)
      )

      last_team = Startup.last

      # New startup has expected properties.
      expect(last_team.founders.pluck(:id)).to match_array(
        founders_in_same_level.pluck(:id)
      )
      expect(last_team.name).to eq(team_name)

      expect(founder_1.reload.startup.founders.count).to eq(4)
      expect(founder_1.startup.faculty.pluck(:id)).to match_array(
        [faculty_1.id, faculty_2.id]
      )
    end

    it 'raises an exception when founders belong to different levels' do
      expect {
        subject.new(founders_in_different_levels).team_up(team_name)
      }.to raise_error(
        RuntimeError,
        'Students must belong to the same level for teaming up'
      )
    end

    context 'when teams have some tags' do
      let(:startup_1) do
        create :startup, level: level_1, tag_list: %w[tag_a tag_b]
      end
      let(:startup_2) { create :startup, level: level_1, tag_list: %w[tag_c] }

      it 'copies the tags to a student who moves out' do
        expect do
          subject.new(Founder.where(id: founder_1.id)).team_up('foo')
        end.to(change { Startup.count }.from(3).to(4))
