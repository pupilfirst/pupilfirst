...
let canBeMarkedActive = (selectedStudents, students) => {
 switch(ListUtils.isEmpty(selectedStudents)) {
 | true => false
 | false => {
  let teamId = selectedStudents |> List.hd |> Student.teamId;
  let selectedAllStudents = (selectedStudents |> List.length) === (students |> List.filter(student => Student.teamId(student) === teamId) |> List.length );
  (selectedStudents |> List.for_all(student => Student.teamId(student) === teamId)) && selectedAllStudents;
 }
 }
};
