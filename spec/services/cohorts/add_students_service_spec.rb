...
describe Cohorts::AddStudentsService do
  ...
  describe '#add' do
    it 'add given list of students to the course' do
      ...
      expect { subject.add(students_data) }.to change { course.founders.count }
        .by(4)
      ...
      expect(email_subject).to eq(
        "You have been added as a student in #{course.school.name}"
      )
    ...
    context 'when the data includes a student alone in a team' do
      ...
      it 'onboards the student as a "standard" student' do
        ...
        expect(student_user.founders.first.startup.name).to eq(
          student_user.name
        )
      end
    end
