module Users
  class AnalyticsStateService
    ...
    def fee_payment_complete?
      Target.find_by(key: Target::KEY_FEE_PAYMENT).status(@user.founder) == Targets::STATUS_COMPLETE
    end
    ...
    def screening_complete?
      Target.find_by(key: Target::KEY_SCREENING).status(@user.founder) == Targets::STATUS_COMPLETE
    end
  end
end
