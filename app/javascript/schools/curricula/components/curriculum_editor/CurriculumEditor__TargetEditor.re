...
let component = ReasonReact.reducerComponent("CurriculumEditor__TargetEditor");
...
let createTarget = (state, authenticityToken, targetGroupId) => {
  ...
  Js.Dict.set(payload, "slideshow_embed", state.slideshowEmbed |> Js.Json.string);
  Js.Dict.set(payload, "resource_ids", resourceIds |> Json.Encode.(list(int)));

  Js.Dict.set(payload, "prerequisite_target_ids", prerequisiteTargetIds |> Json.Encode.(list(int)));

  Js.Dict.set(payload, "quiz", state.quiz |> Json.Encode.(list(QuizQuestion.encoder)));

  switch (state.methodOfCompletion) {
  | Evaluated => Js.Dict.set(payload, "evaluation_criterion_ids", evaluationCriteriaIds |> Json.Encode.(list(int)))
  | VisitLink => Js.Dict.set(payload, "link_to_complete", state.linkToComplete |> Js.Json.string)
  | TakeQuiz => ()
  | MarkAsComplete => ()
  | NotSelected => ()
  };
...
let make = (~targetGroupId, ~evaluationCriteria, ~targets, ~authenticityToken, ~hideEditorActionCB, _children) => {
  ...component,
  initialState: () => {
    title: "",
    description: "",
    ...
    evaluationCriteria:
      evaluationCriteria
      |> List.map(criteria => (criteria |> EvaluationCriteria.id, criteria |> EvaluationCriteria.name, true)),
    prerequisiteTargets:
      targets
      |> List.filter(target => target |> Target.targetGroupId == 1)
      |> List.map(_target => (_target |> Target.id, _target |> Target.title, false)),
  ...
  reducer: (action, state) =>
    ...
    | UpdateState(keyForStateUpdation, string) =>
      ...
      | SlideshowEmbed => ReasonReact.Update({...state, slideshowEmbed: string})
      | LinkToComplete => ReasonReact.Update({...state, linkToComplete: string})
      }
    | UpdateEvaluationCriterion(key, value, selected) =>
      let oldEC = state.evaluationCriteria |> List.filter(((item, _, _)) => item !== key);
      ReasonReact.Update({...state, evaluationCriteria: [(key, value, selected), ...oldEC]});
    | UpdatePrerequisiteTargets(key, value, selected) =>
      let oldPT = state.prerequisiteTargets |> List.filter(((item, _, _)) => item !== key);
      ReasonReact.Update({...state, prerequisiteTargets: [(key, value, selected), ...oldPT]});
    | UpdateMethodOfCompletion(methodOfCompletion) => ReasonReact.Update({...state, methodOfCompletion})
    | AddQuizQuestion =>
      let lastQuestionId = state.quiz |> List.rev |> List.hd |> QuizQuestion.id;
      ReasonReact.Update({
        ...state,
        quiz: state.quiz |> List.rev |> List.append([QuizQuestion.empty(lastQuestionId + 1)]) |> List.rev,
      });
    | UpdateQuizQuestion(id, quizQuestion) =>
      let newQuiz = state.quiz |> List.map(a => a |> QuizQuestion.id == id ? quizQuestion : a);
      ReasonReact.Update({...state, quiz: newQuiz});

    | RemoveQuizQuestion(id) =>
      ReasonReact.Update({...state, quiz: state.quiz |> List.filter(a => a |> QuizQuestion.id !== id)})
    | AddResource(key, value) => ReasonReact.Update({...state, resources: [(key, value), ...state.resources]})
    | RemoveResource(key) =>
      let newResources = state.resources |> List.filter(((_key, _)) => _key !== key);
  ...
  render: ({state, send}) => {
    ...
    let updateQuizQuestionCB = (id, quizQuestion) => send(UpdateQuizQuestion(id, quizQuestion));
    let questionCanBeRemoved = state.quiz |> List.length > 1;
    let addResourceCB = (key, value) => send(AddResource(key, value));
    let isValidQuiz =
      state.quiz
      |> List.filter(quizQuestion => quizQuestion |> QuizQuestion.isValidQuizQuestion != true)
      |> List.length == 0;
      ...
      <div className="drawer-right">
        <div className="drawer-right-form w-full">
          <div className="w-full">
            <div className="mx-auto bg-white">
              <div className="max-w-md p-6 mx-auto">
                ...
                <input
                  ...
                  onChange={event => send(UpdateState(VideoEmbed, ReactEvent.Form.target(event)##value))}
                />
                <label className="block tracking-wide text-grey-darker text-xs font-semibold mb-2" htmlFor="title">
                  {"Embed Slideshow" |> str}
                </label>
                <input
                  ...
                  onChange={event => send(UpdateState(SlideshowEmbed, ReactEvent.Form.target(event)##value))}
                />
                <label className="block tracking-wide text-grey-darker text-xs font-semibold mb-2" htmlFor="title">
                  {"Resources" |> str}
                </label>
              ...
              <div className="max-w-md p-6 mx-auto">
                ...
                <div className="flex items-center mb-6">
                  ...
                  <div className="inline-flex w-64 rounded-lg overflow-hidden border">
                    ...
                    <button
                      ...
                      className={facultyReviewButtonClasses(
                        state.methodOfCompletion != Evaluated && state.methodOfCompletion != NotSelected,
                      )}>
                      {"No" |> str}
                    </button>
                  </div>
                       ...
                       <div className="flex -mx-2">
                         <div className="w-1/3 px-2">
                           <button
                             ...
                             className={completionButtonClasses(state.methodOfCompletion == MarkAsComplete)}>
                             <svg
                               ...
                               <g id="bfe57d46-3bb7-42f0-b4e6-eccc9a02f1dd">
                                 <g id="ea523c13-a500-4391-9a0e-cf57c4f4a027">
                                   ...
                                   />
                                   <path d="M16.17,6.87H8.32a.47.47,0,0,0,0,.93h7.85a.47.47,0,0,0,0-.93Z" />
                                   <path
                         ...
                         <div className="w-1/3 px-2">
                           <button
                             ...
                             className={completionButtonClasses(state.methodOfCompletion == VisitLink)}>
                             <svg
                               className="fill-current w-8 mb-2"
                               xmlns="http://www.w3.org/2000/svg"
                               viewBox="0 0 24 29.4">
                         ...
                         <div className="w-1/3 px-2">
                           <button
                             ...
                             className={completionButtonClasses(state.methodOfCompletion == TakeQuiz)}>
                             <svg
                               className="fill-current w-8 mb-2"
                               xmlns="http://www.w3.org/2000/svg"
                               viewBox="0 0 24 29.4">
                ...
                {switch (state.methodOfCompletion) {
                 ...
                 | VisitLink =>
                   <div>
                     <label
                       className="block tracking-wide text-grey-darker text-xs font-semibold mb-2" htmlFor="title">
                     ...
                     <input
                       ...
                       onChange={event => send(UpdateState(LinkToComplete, ReactEvent.Form.target(event)##value))}
                     />
                   </div>
            ...
            <div className="flex">
              <button
                onClick={_event => createTarget(state, authenticityToken, targetGroupId)}
