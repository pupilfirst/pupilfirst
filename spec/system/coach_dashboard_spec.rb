...
feature 'Coach Dashboard' do
  ...
  context 'with multiple pending submissions' do
  ...
  context 'with only one pending submission' do
    let(:submission) { create(:timeline_event, latest: true, target: target) }
    let(:student) { startup_1.founders.first }
    ...
    scenario 'coach sends some feedback', js: true do
      ...
      # Check the subject of the mail, and whether there is a link to the target inside it.
      expect(current_email.subject).to include("New feedback from #{coach.name} on your submission")
      expect(current_email.body).to include("/targets/#{target.id}")
    end
  end
end
