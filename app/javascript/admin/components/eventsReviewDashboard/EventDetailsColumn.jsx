...
export default class EventDetailsColumn extends React.Component {
  ...
  linkTarget() {
    ...
    const selectedTargetName = $(inputId)
      .find(":selected")
      .text();
    const postUrl =
      "/admin/timeline_events/" + eventId + "/link_target?ajax=true";
    const that = this;
        ...
        success: function() {
          ...
          new PNotify({
            ...
            text:
              "Event " +
              eventId +
              " has been linked to target with ID " +
              selectedTargetId +
              "."
          });
  ...
  render() {
    ...
    return (
      <div>
        <div>
          <a
            href={"/admin/timeline_events/" + this.props.eventId}
            target="_blank"
          >
        ...
        <div>
          ...
          <a
            href={"/admin/founders/" + eventData["founder_id"]}
            target="_blank"
          >
            {eventData["founder_name"]}
          </a>
          &nbsp; (<a
            href={"/admin/startups/" + eventData["startup_id"]}
            target="_blank"
          >
            {eventData["startup_name"]}
          </a>)
          <br />
        ...
        <div>
          <div>
            ...
            {this.isTargetLinked() && (
              <a
                href={"/admin/targets/" + this.state.linkedTarget.id}
                target="_blank"
              >
          ...
          <div>
            <strong>{this.linkTargetLabel()}</strong>
            <br />
            <select id={this.targetInputId()} />
          </div>

          <a
            className={this.linkTargetButtonClasses()}
            onClick={this.linkTarget}
          >
            {this.linkTargetButtonText()}
          </a>
        </div>
        ...
        {eventData["improvement_of"] && (
          <div className="margin-top-10">
            ...
            <a
              href={
                "/admin/timeline_events/" + eventData["improvement_of"]["id"]
              }
              target="_blank"
            >
              {eventData["improvement_of_title"] +
                " (" +
                eventData["improvement_of"]["status"] +
                ")"}
            </a>
            <br />
          </div>
                ...
                <div key={file.id + file.title}>
                  <i className="fa fa-file" />&nbsp;<a
                    href={
                      ...
                      file.timeline_event_id +
                      "/get_attachment?timeline_event_file_id=" +
                      file.id
                    }
                    target="_blank"
                  >
        ...
        {eventData["image"] && (
          <div>
            <i className="fa fa-file-image-o" />&nbsp;<a
              href={
                "/admin/timeline_events/" + this.props.eventId + "/get_image"
              }
              target="_blank"
            >
              {eventData["image"]}
