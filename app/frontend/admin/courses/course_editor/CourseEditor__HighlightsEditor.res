...
let selected = (highlight: Course.Highlight.t) => {
  <button
    ...
    className="flex items-center justify-center cursor-pointer bg-white border border-gray-300 text-gray-900 rounded-lg p-3 w-12 h-12 ltr:mr-1 rtl:ml-1 hover:bg-primary-100 hover:text-primary-400 hover:border-primary-400 focus:outline-none focus:bg-primary-100 focus:text-primary-400 focus:border-primary-400">
  ...
  <div> {Js.Array.mapi((highlight, index) => {
      ...
      <Spread props={"data-highlight-index": index} key={string_of_int(index)}>
        <div key={string_of_int(index)} className="flex items-start py-2 relative">
          <div className="flex items-start w-full bg-gray-50 border rounded-lg p-4 ltr:mr-1 rtl:ml-1">
            <Dropdown2
