...
feature 'Certificate verification', js: true do
  ...
  around { |example| Time.use_zone(user&.time_zone || 'UTC') { example.run } }
  ...
  scenario 'user verifies certificate' do
    ...
    expect(page).not_to have_text(
      "This student's name was updated after the certificate was issued."
    )
  end

  scenario 'a member of the public verifies the certificate' do
    visit issued_certificate_path(
            serial_number: issued_certificate.serial_number
          )
  ...
  context 'when the user changes her name after the certificate is issued' do
    let!(:issued_certificate) do
      create :issued_certificate, name: Faker::Name.name
    end
  ...
  context 'user is not present' do
    let!(:issued_certificate) do
      create :issued_certificate, name: Faker::Name.name, user: nil
    end
